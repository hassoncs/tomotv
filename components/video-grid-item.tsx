...
function VideoGridItemComponent({video, onPress, index}: VideoGridItemProps) {
  ...
  const posterUrl = useMemo(() => (hasPoster(video) ? getPosterUrl(video.Id, 600) : undefined), [video])

  const duration = useMemo(() => formatDuration(video.RunTimeTicks), [video.RunTimeTicks])
