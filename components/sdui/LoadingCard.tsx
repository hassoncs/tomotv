...
export const loadingCardPropsSchema = z.object({
  ...
  estimatedDurationSeconds: z
    ...
    .positive()
    .optional()
    .describe("If provided, animates the determinate progress bar from `progress` to 100% over this many seconds. Requires `progress` to be set."),
...
export function LoadingCard({ title, subtitle, progress, estimatedDurationSeconds, cancellable = false, component = "LoadingCard" }: LoadingCardProps) {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.row}>
        {!hasDeterminateBar && <ActivityIndicator size={TV ? "large" : "small"} color="#FFC312" />}
      ...
      {hasDeterminateBar && <AnimatedProgressBar progress={progress!} durationToCompleteSeconds={estimatedDurationSeconds} />}
