...
export function EpisodeList({ episodes, seriesTitle, component = "EpisodeList" }: EpisodeListProps) {
  ...
  const renderItem = ({ item }: { item: Episode }) => {
    ...
    const epLabel =
      item.seasonNumber !== undefined && item.episodeNumber !== undefined ? `S${item.seasonNumber}E${item.episodeNumber}` : item.episodeNumber !== undefined ? `E${item.episodeNumber}` : "";
    ...
    return (
      <TouchableOpacity
        ...
        {item.durationMinutes ? <Text style={styles.duration}>{item.durationMinutes}m</Text> : null}
      </TouchableOpacity>
    );
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList data={episodes} keyExtractor={(item) => item.id} renderItem={renderItem} contentContainerStyle={styles.list} scrollEnabled={false} removeClippedSubviews={false} />
    </View>
  );
}
