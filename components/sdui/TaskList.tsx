...
const taskItemSchema = z.object({
  id: z.string().describe(
    'Stable unique ID for this task — used as React key so state is preserved across re-renders. Use snake_case e.g. "search_library", "download_ep".',
  ),
  label: z.string().describe(
    'Short task description shown to the user. e.g. "Search library for Alone"',
  ),
  status: z
  ...
  detail: z
    .string()
    .optional()
    .describe('Optional secondary line below the label. e.g. "Found 8 seasons" or "Starting in 2 min"'),
  progress: z
    .number()
    .min(0)
    .max(1)
    .optional()
    .describe('Progress 0–1 for a determinate operation (e.g. download). Shows AnimatedProgressBar. Omit for no bar.'),
  progressPerSecond: z
    .number()
    .positive()
    .optional()
    .describe('Rate of progress per second. Drives AnimatedProgressBar forward without re-renders. e.g. 0.01 = 1%/s.'),
});
...
export const taskListPropsSchema = z.object({
  title: z
    .string()
    .optional()
    .describe('Optional header above the list. e.g. "Working on it..." or "All done!"'),
  tasks: z
    .array(taskItemSchema)
    .min(1)
    .describe(
      'Ordered list of tasks. Always include ALL tasks on every update — only status/detail/progress change.',
    ),
});
