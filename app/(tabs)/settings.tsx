...
import {Host, SecureField, Switch, TextField, TextFieldRef} from "@expo/ui/swift-ui"
...
export default function SettingsScreen() {
  ...
  const [serverProtocol, setServerProtocol] = useState<"http" | "https">(initialEnv.protocol)
  ...
  const loadSettings = async () => {
    try {
      setIsLoading(true)
      const [savedIp, savedPort, savedProtocol, savedKey, savedUserId, savedQuality] = await Promise.all([
  ...
  const validateInputs = (): {valid: boolean; error?: string} => {
    ...
    if (
      ...
      return {
        valid: false,
        error: "Invalid server IP format. Use format: 192.168.1.100 or localhost (no port)"
      }
    }
    ...
    if (
      ...
      !/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/i.test(userIdTrimmed)
    ) {
      return {
        valid: false,
  ...
  const testConnection = async () => {
    try {
      ...
      const sanitizedServerIp = currentServerIp.current.trim().replace(/[<>'"]/g, "")
      const sanitizedServerPort = currentServerPort.current.trim().replace(/[^0-9]/g, "")
              ...
              <View style={styles.inputContainer}>
                ...
                <Host matchContents>
                  <Switch
                    ...
                    onValueChange={value => setServerProtocol(value ? "https" : "http")}
                    label="Use HTTPS"
                    variant="switch"
                    systemImage={serverProtocol === "https" ? "lock.fill" : "lock.open"}
                  />
                </Host>
              </View>
            </View>
