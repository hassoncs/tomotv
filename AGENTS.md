...
Never commit secrets; rely on secure store APIs and Expo config values. `scripts/update-dev-ip.js` runs via npm hooks to write your active LAN IP (fallback to `localhost`), so rerun it manually only if you switch networks mid-session. For TV builds, set `EXPO_TV=1` locally and verify the Apple/Android TV asset sets (`TVOS_ICONS.md`, `Images.xcassets/`) stay in sync with feature work.
